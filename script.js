...
function addToCart(name, price) {
  ...
  if (existingItem) {
    ...
    cart.push({
      name,
      price,
      quantity: 1, // Se não estiver no carrinho, adiciona com quantidade 1
    });
  }

  updateCartModal(); // Atualiza o modal com os novos dados do carrinho
}
...
function updateCartModal() {
  ...
  // Exibe todos os itens no carrinho com suas quantidades
  cart.forEach(item => {
    ...
    cartItemElement.innerHTML = `
      <div class="flex items-center justify-between">
        <div>
          ...
          <p>Qtd: ${item.quantity}</p> <!-- Exibe a quantidade do item -->
...
function removeItemCart(name) {
  ...
  if (index !== -1) {
    ...
    if (item.quantity > 1) {
      ...
      updateCartModal(); // Atualiza o modal com a nova quantidade
      return;
    }
    cart.splice(index, 1); // Se a quantidade for 1, remove o item do carrinho
    updateCartModal(); // Atualiza o modal após remover o item
  }
}
...
checkoutBtn.addEventListener("click", function () {
  ...
  if (!isOpen) {
    Toastify({
      ...
      stopOnFocus: true, // Impede o dismiss ao passar o mouse
      style: {
        background: "#ef4444",
      },
    }).showToast();
  ...
  const phone = "24999996655";

  window.open(`https://wa.me/${phone}?text=${message} Endereço: ${addressInput.value}`, "_blank");

  // Limpar o carrinho após o pedido
  cart = [];
  updateCartModal(); // Atualiza o modal após limpar o carrinho
});
